...
class DIISCO:
    ...
    def sample(
        ...
        while samples_so_far < n_samples:
            ...
            all_train_signals = torch.cat(
                [self.train_signals, self.train_signals],
                dim=0,
            )
            ...
            f_mean, f_cov = prior_updated_model.predict(timepoints, signals)
    ...
    def fit_and_set_f_prior_params(
        ...
        print("Set prior parameters: mean " + str(self.prior_f_mean) + ", cov " + str(self.prior_f_cov))
    ...
    def get_f_mean(self, n_samples: int = 1) -> torch.Tensor:
        ...
        for _ in range(n_samples):
            guide_trace = poutine.trace(self.guide).get_trace(
                self.train_timepoints, self.train_signals
            )
            trained_model = poutine.replay(self._model, trace=guide_trace)
            model_trace = poutine.trace(trained_model).get_trace(
                self.train_timepoints, self.train_signals
