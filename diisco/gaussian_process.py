...
class GaussianProcessRegressorDoubleSep:
    ...
    def predict(self, t, s):
        ...
        assert var.shape == (t.shape[0], t.shape[0])
        return mean, var

    def sample(self, t, s, n_samples=1, with_cov=False):
        """
        Sample the output for the given input data.
        :param X: The input data. shape (n_samples, n_features)
        :param n_samples: The number of samples to draw. Default is 1.
        ...
        samples = samples.view(n_samples, t.shape[0], self.y.shape[1])
        assert samples.shape == (
            n_samples,
            t.shape[0],
            self.y.shape[1],
